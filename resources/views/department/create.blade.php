@if(isset($department->id))
                <form id="addform" method="post" enctype="multipart/form-data" onsubmit="return saveModalFrm(this);"  action="{{route('department.update',$department->id)}}" method="POST"> 
                <input type="hidden" value="PATCH" name="_method">    
            @else
                <form id="addform" method="POST" onsubmit ="return saveModalFrm(this);" enctype="multipart/form-data" action="{{route('department.store')}}" method="POST"> 
				...
				<div class="m-form__section m-form__section--first">
					...
					<div class="form-group m-form__group row">
						...
						<div class="col-lg-9 m-form__group-sub ">
							<input type="text" class="form-control m-input m-input--square" id="name" name="name" placeholder="{{ __('messages.entername')}}" data-msg-required="This field is required." value="{{ ((isset($department)) ? $department->name : old('name')) }}" pattern="[a-z{1}[A-Z]{1}[0-9]{1}]" data-msg-pattern="Please enter valid email" required />
